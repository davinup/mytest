...
data_set=pd.read_csv('iris.csv',header=0)
train_set,test_set=randsplit(data_set,0.8)
result_data=gnb_classify(train_set,test_set)
print(result_data.iloc[:,-2:])

# item = train_set.loc[train_set.iloc[:, -1] == 'setosa', :]  # 分别提取出每一种类别
# m = item.iloc[:, :-1].mean()  # 当前类别的平均值
# print(m)
# s = np.sum((item.iloc[:,:-1] - np.tile(m,(item.shape[0],4))) ** 2) / (item.shape[0])  # 当前类别的方差
# m=gnb_classify(train_set,test_set)
